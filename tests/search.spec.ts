...
test.describe('Search Functionality', () => {
	...
	test('End-to-end: should allow the user to search for a city and display weather information', async ({
		...
		await page.waitForSelector('text=Copenhagen, DK', { timeout: 2000 });
		await page
			.getByRole('option', { name: 'Copenhagen, DK' })
			.click({ force: true });
		await page.waitForSelector('text=AIR CONDITIONS', { timeout: 2000 });
		// await page.screenshot({ path: 'debug-screenshot.png' });
		await expect(page.locator('text="AIR CONDITIONS"')).toHaveCount(1);
	});
});
